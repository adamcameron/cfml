...
zingchart = {
	...
	jscsvfunc : function(sJson) {
		var oResponse = eval('(' + sJson + ')');
		var sId = oResponse['id'] || '';
		return zingchart.oCsv[sId];
	},
	...
	render_flash : function(oOptions) {
		...
		if (oOptions['safe-render']) {
			...
			function _create_detector_() {
				...
				swfobject.embedSWF(
					sLibUrl.replace(sSwfName, 'detector.swf'),
					'zc_detector',
					1,
					1,
					'10',
            ...
            var sSwfName = sLibUrl.substring(sLibUrl.lastIndexOf('/')+1);
		...
		var sCsv = oOptions['csvdata'] || '';
		var sDefaults = oOptions['defaults'] || '';
		sDefaults = (sDefaults.replace(/(\s)/,'')=='')?'{}':sDefaults;
		...
		for (key in oOptions)
        {
            if (key == "allowlocal" || key == "mode" || key == "exportdataurl" || key == "exportimageurl" || key == "exportpdfurl" || key == "errormessage" 
             || key == 'customprogresslogo' || key == 'hideprogresslogo' || key == 'hideprogressbar' || key == 'progressbarcolor' || key == 'progressbordercolor' || key == 'showwatermark'){
                oFlashVars[key] = oOptions[key];
             }
             else if (key == "progressbar"){
                 for (pbkey in oOptions[key]){
                    oFlashVars["pb" + pbkey] = oOptions[key][pbkey];
                 }
             }
        }
		...
		if (sCsv != ''){
		    oFlashVars['jscsvfunc'] = 'zingchart.jscsvfunc';
		    zingchart.oCsv[sId] = sCsv;
		}
		if (sDefaults != ''){
