component {
	...
	static function runTests(functionToTest, testSet){
		var testsToRun = isNull(testSet) ? static.tests : testSet.map(function(i){
			return static.tests[i]
		})
		...
		testsToRun.each(function(test, i){
			try {
				...
				LuceeTester::outputMessage(message)
			}
		})
	}
