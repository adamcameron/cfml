<cffunction name="htmlEntityToUTF8Original" output="false" access="public" returntype="any" hint="">
	<cfargument name="source" type="string" required="true">
	<cfset var idx = 0>
	<!--- NOTE : Does not convert the "160,nbsp". You can add it yourself if you need to. --->
	<cfset var ents = [
	...
	<cfloop from="1" to="#arrayLen(ents)#" index="idx" step="2">
		<cfset arguments.source = replaceNoCase(arguments.source, "&#ents[idx+1]#;", chr(ents[idx]), "all")>	
	</cfloop>
...
string function htmlEntityToUTF8(required string source){
	...
	s = "";
	for(i=1; i <= arrayLen(ents); i+=2){
		s &= "&#ents[i+1]#; ";
	}
</cfscript>
<table border="1">
	<thead>
		<tr><th>Old</th><th>New</th><th>Ucase</th><th>Equals</th></tr>
	</thead>
	<tbody>
		<cfset cOld = htmlEntityToUTF8Original(s)>
		<cfset cNew = htmlEntityToUTF8(s)>
		<cfoutput>
			<tr>
			<td>#cOld#</td>
			<td>#cNew#</td>
			<td>#uCase(cNew)#</td>
			<td>#compare(cOld, cNew)#</td>
			</tr>
		</cfoutput>
	</tbody>
</table>